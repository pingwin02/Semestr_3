...
int drawGLScene(int counter)
{
    ...
    glDrawArrays(GL_TRIANGLE_STRIP, 0, 40); //rysowanie trojkata

    glBindVertexArray(vao[1]);
    glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, 0); //rysowanie prostokata

    translationMatrix = glm::translate(glm::mat4(), glm::vec3(0.0f, 0.5f, 0.5f));
    scaleMatrix = glm::scale(glm::mat4(), glm::vec3(0.5f, 1.0f, 0.5f));
  ...
  translationMatrix = glm::translate(glm::mat4(), glm::vec3(0.0f, 1.0f, 0.25f));
      scaleMatrix = glm::scale(glm::mat4(), glm::vec3(0.25f, 1.0f, 0.25f));
      rotationMatrix = glm::rotate(glm::mat4(), glm::radians(fi), glm::vec3(0.0f, 1.0f, 0.0f));
      transformMatrix = projectionMatrix * viewMatrix * translationMatrix * rotationMatrix * scaleMatrix;
    ...
    translationMatrix = glm::translate(glm::mat4(), glm::vec3(0.0f, 1.5f, 0.12f));
      scaleMatrix = glm::scale(glm::mat4(), glm::vec3(0.15f, 1.0f, 0.12f));
      rotationMatrix = glm::rotate(glm::mat4(), glm::radians(-fi), glm::vec3(0.0f, 1.0f, 0.0f));
      transformMatrix = projectionMatrix * viewMatrix * translationMatrix * rotationMatrix * scaleMatrix;
